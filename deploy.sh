...
#     · git（GitHub）是【唯一真相源】；ECS（网站目录）只是「由本脚本
...
#     · 为什么不让 git 同步自 ECS：ECS 是最终结果，但必须由 git 生成，
#           不能反过来把「服务器上的临时/半成品状态」固化进历史——
#           否则 git 丧失可审查、可回滚能力，且两个写入源互相打架
#           （正是 2026-07-24 那 4 个 pxid 页被冲掉的根因）。
#     · 类比：ECS = 编译出的 .exe（交付物），git = 源码 .c（真相源）。
#           你不会去改 .exe 再同步回 .c，而是改源码、重新编译、重新部署。
# ══════════════════════════════════════════════════════════════════
set -euo pipefail
